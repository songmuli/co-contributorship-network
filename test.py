...
print("数据库连接成功！")
cursor = db.cursor()
cursor.execute('SELECT DISTINCT doi FROM contrib_pbio;')
...
for Doi in result_doi:
  ...
  doi2 = str_doi[-7:]
  doi = doi1 + doi2
  cursor.execute("SELECT DISTINCT contribution_type FROM contrib_pbio WHERE doi = %s", Doi)
  contrib_type = []
  ...
  for con in contrib_type:
   ...
   cursor.execute('SELECT author_name FROM contrib_pbio where (doi = %s) & (contribution_type = %s)', (Doi, con))
   ...
   if len(contrib_authors) != 1: #如果作者数不等于1，就进行两两组合组合
       ...
       # G.add_edges_from(com_list)
  all_contrib_author += new_contrib_authors #
